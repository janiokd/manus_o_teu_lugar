...
export default function PropertyDetailPage() {
  ...
  const loadProperty = async (propertyId: string) => {
    try {
      setLoading(true);
      console.log('Carregando imóvel com ID:', propertyId);
          ...
          <Grid item xs={4} md={4} spacing={2}>
            <Stack spacing={2}>
              ...
              <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
                <Stack
                  spacing={2}
                  sx={{ bgcolor: 'common.white', borderRadius: '15px', p: 2.5 }}
                >
                  <Typography variant="h4" color="primary">
                    {t(`buyPage.interested`)}
                  </Typography>
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                    {t(`buyPage.fill_data`)}
                  </Typography>

                  <RHFTextField name="name" label={t(`buyPage.name`)} />
                  <RHFTextField name="email" label={t(`buyPage.email`)} />
                  <RHFTextField name="phoneNumber" label={t(`buyPage.phone_whatsapp`)} />
